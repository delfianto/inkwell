...
describe("Removing Links", () => {
  it("removes wiki links", () => {
    ...
    expect(replaceWikiLinks("[[link]] followed by a single end bracket]")).toEqual(
      "link followed by a single end bracket]",
    );
    expect(replaceWikiLinks("[[Filename|Alias|OtherAlias|Another Alias]]")).toEqual(
      "AliasOtherAliasAnother Alias",
    );
  });
