import { debounce, type Debouncer, type TAbstractFile, TFile, type Vault } from "obsidian";
...
const WORD_COUNT_REGEX = (() => {
  ...
  const nonSpaceDelimitedWords =
    /[\u3041-\u3096\u309D-\u309F\u30A1-\u30FA\u30FC-\u30FF\u4E00-\u9FD5]{1}/u.source;
  return new RegExp(
    [
      `${String.raw`(?:[0-9]+(?:(?:,|\.)[0-9]+)*|[\-` + spaceDelimitedChars}])+`,
...
export class WordCountTracker {
  ...
  private debouncedCountProject: Debouncer<[TAbstractFile, string | null], Promise<void>>;
  ...
  private async countProjectContaining(file: TAbstractFile, oldPath: string | null) {
    ...
    if (oldPath) {
      const previousProject = projectForPath(oldPath, get(projectsStore));
      if (previousProject) {
        const projectCount = await this.countWordsInProject(previousProject);
        projectWordCountsStore.update((counts) => {
          counts[previousProject.vaultPath] = projectCount;
          return counts;
        });
      }
    }
  ...
  fileModified(file: TAbstractFile): void {
    void this.countProjectContaining(file, null);
  }

  debouncedCountProjectContaining(file: TAbstractFile, oldPath: string | null = null): void {
    void this.debouncedCountProject(file, oldPath);
  }
}
